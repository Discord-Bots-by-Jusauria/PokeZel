...
from utilities.pet_interaction import feedView
...
class Pet(commands.Cog):
        ...
        for owner in owners:
            ...
            for stat in list:
                ...
                if pet["is_sleeping"] and stat == "energy":
                    ...
                    continue
                pet[stat] -= int(1* typeEffect * pet["mood"]["generalBuff"][stat]*sicknessDebuff)
            ...
            if result["status"]==0:
                title = f"<@{owner["user_id"]}>. {pet["nickname"]} has taken it's final breath... ⚰️"
    ...
    def update_pet_mood(self,owner,pet, statList):
        ...
        if result["status"] == 1:
            if  pet["mood"]["name"] in [mood["name"] for mood in personality["sick_tendency"]]:
                return {"status":1,"description":description}
            for sick_mood in personality["sick_tendency"]:
                if random.randint(0,100)<sick_mood["chance"]:
                    ...
                    return {"status":1,"description":(description+moodscription)}
        ...
        if pet["sick"]:
            if pet["sick"]["name"]=="Dying":
                if (datetime.fromtimestamp(logs["dying"]) + timedelta(minutes=30)) <= datetime.now() and pet["health"] == 0:
                    ...
                    description += get_messages("death",pet["nickname"])
                    return {"status":0, "message":random.choice(description)}
        ...
        for stat in statList:
            ...
            if pet[stat] <= 10 and stat in ["hunger", "thirst"]:
                low_eat_sicknesses.extend(
                    s for s in sicknesses
                    if any(key in s.get("triggers", {}) for key in ["eat", "drink", "happy"])
                    and any(s["triggers"].get(key, 100) <= 10 and s["triggers"].get(key, 100) > 0 for key in ["eat", "drink", "happy"])
                )
